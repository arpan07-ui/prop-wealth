...
import { FALLBACK_FIRMS, withTimeout } from './fallbackData';
...
export const mapFirmFromDB = (dbFirm: any): PropFirm => {
    ...
    return {
        ...
        discountCode: (dbFirm.discount_code === 'SPOT' || dbFirm.discount_code === 'PropMatchSpot' || dbFirm.discount_code === 'WEALTH' ? 'WEALTHX' : dbFirm.discount_code) || 'WEALTHX',
    ...
    async getActiveFirms(mode?: 'forex' | 'futures' | 'crypto'): Promise<PropFirm[]> {
        ...
        try {
            ...
            if (res?.data && res.data.length > 0) {
                ...
                rawData = FALLBACK_FIRMS;
            }
        } catch (err) {
            console.warn('Using resilient directory for active firms:', err);
            rawData = FALLBACK_FIRMS;
        }

        const mappedFirms = rawData.map(mapFirmFromDB);

        if (!mode) return mappedFirms;

        // Filter in JS for resilience and strict isolation
        const filtered = mappedFirms.filter(firm => {
        ...
        // If mode-specific filter returned 0, return all mapped firms rather than a blank screen
        return filtered.length > 0 ? filtered : mappedFirms;
    },
