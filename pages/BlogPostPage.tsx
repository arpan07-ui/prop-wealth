...
const BlogPostPage: React.FC = () => {
    ...
    const fetchPost = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          document.title = data.meta_title || `${data.title} | PROPxWEALTH Blog`;
  ...
  return (
    <div className="min-h-screen bg-brand-black text-white font-sans pt-16">
      <SEO 
        title={post.meta_title || `${post.title} | PROPxWEALTH Blog`}
        description={post.meta_description || post.excerpt}
        keywords={post.tags?.length ? post.tags.join(', ') : 'prop trading, funded account, prop firm evaluation'}
        ...
        jsonLd={{
          ...
          'author': {
            '@type': 'Person',
            'name': post.author || 'PROPxWEALTH Editorial Team'
          },
      ...
      <div className="max-w-[800px] mx-auto px-4 sm:px-6 -mt-8 relative z-10">
        ...
        <div className="bg-gradient-to-r from-brand-gold/10 to-transparent border border-brand-gold/20 rounded-2xl p-8 mb-16 text-center">
          ...
          <p className="text-brand-muted mb-6 max-w-md mx-auto">Compare firms, read verified reviews, and get exclusive discounts. Use code <strong className="text-brand-gold">WEALTH</strong> to save!</p>
