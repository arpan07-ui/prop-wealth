...
import { FirmService, mapFirmFromDB } from '../lib/services';
import { PropFirm } from '../types';
import { useTradeMode } from '../context/TradeModeContext';
import { FALLBACK_FIRMS } from '../lib/fallbackData';

const BrowseFirmsPage: React.FC = () => {
  const { mode } = useTradeMode();
  const [firms, setFirms] = useState<PropFirm[]>(FALLBACK_FIRMS.map(mapFirmFromDB));
  ...
  useEffect(() => {
    const fetchFirms = async () => {
      try {
        ...
        console.warn('Failed to load firms, keeping fallback directory', err);
      } finally {
        setLoading(false);
      }
    };
